...
const FFT = ({ wave, freqDiv, setFc }) => {
	...
	const data: number[] = useMemo(() => {
		...
		for (let i = 0; i < fftSize; i++) {
			mag[i] = Math.sqrt(
				fftOut[2 * i] * fftOut[2 * i] + fftOut[2 * i + 1] * fftOut[2 * i + 1],
			);
		}
...
function DataTable({ flickerIndex, fluDepth, fc }) {
	const rows = [[ Math.round(flickerIndex), Math.round(fluDepth * 100), Math.round(fc) ]];
  return (
	...
	return (
		<Container maxWidth="md">
			...
			<Box position="relative" sx={{ flexGrow: 1 }}>
				...
				<Stack sx={{ marginTop: 1, marginBottom: 2, paddingLeft: 4, paddingBottom: 1 }} spacing={2} direction="row">
				...
				<DataTable flickerIndex={data.flickerIndex} fluDepth={data.fluDepth} fc={fc} />
			</Box>
		</Container>
	);
}
